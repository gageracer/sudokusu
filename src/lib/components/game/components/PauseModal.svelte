...
let currentMode: number | null = $state(null)
    ...
    <div class="bg-white dark:bg-gray-800 max-h-[90vh] overflow-y-auto p-6 rounded-lg text-center min-w-[300px] max-w-2xl w-full mx-4">
        {#if !showStats}
            ...
            <div class="flex flex-col gap-3">
                <button
                    class="rounded bg-sky-600 px-4 py-2 text-white hover:bg-sky-700 transition-colors"
                    onclick={onResume}
                >
                    Resume Game
                </button>
                <button
                    class="rounded bg-green-600 px-4 py-2 text-white hover:bg-green-700 transition-colors"
                    onclick={() => showStats = true}
                >
                    Statistics
                </button>
                <button
                    class="rounded bg-yellow-600 px-4 py-2 text-white hover:bg-yellow-700 transition-colors"
                    onclick={onNewGame}
                >
                    New Game
                </button>
                <button
                    class="rounded bg-red-600 px-4 py-2 text-white hover:bg-red-700 transition-colors"
                    onclick={handleReset}
                >
                    Reset Game
                </button>
